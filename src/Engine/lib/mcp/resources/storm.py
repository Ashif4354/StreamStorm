...
async def get_storm_channels() -> dict[str, Any]:
    ...
    return {
        "channels": ss_instance.all_channels,
        "total_channels": ss_instance.total_channels,
        "storm_running": True
    }
...
async def get_active_channels() -> dict[str, Any]:
    ...
    active: list[int] = []
    for index_str, channel_data in ss_instance.all_channels.items():
...
async def get_storm_messages() -> dict[str, Any]:
    ...
    return {
        "messages": ss_instance.messages,
        "count": len(ss_instance.messages),
        "storm_running": True
    }
